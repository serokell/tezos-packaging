...
replace_baker_key_options = {
    "yes": "Replace existing baker key",
    "no": "Use existing baker key",
}


def get_replace_baker_key_query(config, baker_key_value):
    baker_alias = config["baker_alias"]
    value, address = baker_key_value
    return Step(
        id="replace_baker_key",
        prompt=f"An account with the `{baker_alias}` alias already exists.\n"
        f"Its current value is {value}.\n"
        f"With a corresponding address: {address}\n"
        "Would you like to import a new key and replace this one?",
        help="If you choose 'yes', the existing key will be replaced with the new one.\n",
        options=replace_baker_key_options,
