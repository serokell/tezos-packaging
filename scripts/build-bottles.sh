...
build_bottle () {
    ...
    # brew uninstall "$1"
    FORMULA_FILE_NAME=$(basename $1)
    PACKAGE_NAME=${FORMULA_FILE_NAME%.rb}
    BOTTLE_FILE_NAME=${PACKAGE_NAME}--$VER_PKG.$VER_MACOS.bottle.tar.gz
    echo bla > package
    tar --create --verbose -z -f ./$BOTTLE_FILE_NAME package
}
