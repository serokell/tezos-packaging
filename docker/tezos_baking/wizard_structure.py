...
from tezos_baking.steps_common import Step, get_systemd_service_env, networks, key_import_modes
...
class Setup:
    ...
    def query_step(self, step: Step):
        ...
        additional_steps_to_query = get_tree_path(self.args, step)
        if additional_steps_to_query:
            for (answer, step) in additional_steps_to_query:
                step.process(answer, self.config)
        else:
            if self.args.interactive:
                ...
                if step.default is not None:
                    ...
                    raise ValueError(color(f"Validation error: argument {step.id} is not supplied.", color_red))
    ...
    def import_key(self, key_mode_query, ledger_app=None):
        ...
        while not valid_choice:
            ...
            try:
                self.ensure_step(steps.key_mode_query)
