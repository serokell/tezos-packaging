...
def gen_daemon_specific_postinst(daemon_name):
    ...
    for instance in daemons_instances:
        daemon_postinst += (
            f"systemctl enable {daemon_name}@{instance}.service > /dev/null || true\n"
        )
...
def gen_daemon_specific_postrm(daemon_name):
    ...
    for instance in daemons_instances:
        daemon_postrm += (
            f"systemctl disable {daemon_name}@{instance}.service > /dev/null || true\n"
        )
...
for proto in active_protocols:
    ...
    packages.append(
        OpamBasedPackage(
            ...
            + gen_daemon_specific_postinst(f"tezos-baker-{proto}"),
